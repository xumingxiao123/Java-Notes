...
>**如何验证证书的合法性？浏览器收到服务器返回的证书后, 会验证证书有效性. 验证步骤大概如下:**
>
>- 验证证书**有效期**(起止时间-结束时间)
>- 验证证书**域名**(与浏览器地址栏中域名是否匹配)
>- 验证证书**吊销状态(**CRL+OCSP), [见本文后"吊销检查"章节].
...
数字证书使用数字签名作验证，从Firefox/IE导出的数字证书包含3个部分：证书内容(F)，加密算法(A)，F加密密文(F')，A不是一个算法，而是两个，所以密文F'是F两次加密后的结果。首先，F会被散列算法SHA1计算出hash值h1（称为128bit的摘要），然后h1会被发布这个数字签名的CA认证机构的用私钥进行RSA加密，注意：是发布这个数字签名的CA认证机构，如果现在被加密的数字证书是属于二级CA认证机构的，那么用来加密这个证书的私钥是根CA认证机构的私钥！！RAS加密完后，就形成密文F'。
...
>一个数字证书包括：证书内容F，加密算法A（有两个），加密密后的密文F''
>
>**生成过程**：F-(签名)->F'-（上一层的私钥）->F''
>
>**验证过程**：F-(签名)->F'      F''-(上一层的私钥)->F'  验证F'是否相等，**使用的是签名再加密是为了性能**
>
>如果是假的，那么一定是被篡改了，或者证书无法获取上层的公钥。

**完整性验证**：在验证可信性的过程中，h1和h2的比较，其实已经顺便验证其完整性了。

**有效性验证**：比如验证它的Vilidity值，看是否过期...
...
>
> **计算机资源**：CPU资源，CPU的能力是有限的，占用他的那么他将无心做其他事。
...
![image-20200913162836376](X:\Users\xu\AppData\Roaming\Typora\typora-user-images\image-20200913162836376.png)
...
简单的CLOCK算法是给每一帧关联一个附加位，称为**使用位**。CLOCK算法的性能比较接近LRU，而通过增加使用的位数目，可以使得CLOCK算法更加高效。在使用位的基础上再增加一个**修改位**，则得到改进型的CLOCK置换算法。
...
首先要明白一点：操作系统这块，很多东西都是硬件提供的，软件只是一种抽象描述，硬件如果不提供这种功能，软件基本也白瞎，**内核态~用户态**就是这样一种东西。

这两种状态是CPU的工作状态，**内核态**可以执行更高等级的指令，例如控制磁盘、屏幕等IO，读写整个内存等等，**用户态**只能执行有限的指令，比如+-*/运算，在自己的内存区域里读写所谓的进入内核态，就是CPU进入更高特权级的工作模式。

**这个工作状态的意义在于保证计算机安全运行：**

计算机上电启动时CPU默认在内核态（高特权级），此时CPU控制外存将操作系统读进内存（BootLoader也挺复杂，不过在这不重要），操作系统启动后，还是在内核态，可以干一些初始化的事，比如加载你的显卡驱动，一切都准备好了以后操作系统就可以开始为你干活了：

例如你用键盘输入命令，键盘发出中断信号，CPU对中断信号解析后发现你是要启动《斗地主》游戏，这个时候操作系统将斗地主这个程序的代码读进内存，注意啦！！！此时由内核态切换到用户态，此时斗地主程序代码开始运行，操作系统的代码躺在内存里被雪藏（假设是个单核CPU），当然斗地主只能执行有限的指令，读写有限的内存，它不能直接读写硬盘，也不能直接控制显卡，如果他要发出声音、显示内容，只能调用操作系统的API，这时候会进入内核态，操作系统的代码会被再次运行起来，他会检查斗地主提出的要求是否合法，如果合法，那么就控制声卡显卡动作，如果不合法，通常会杀死斗地主这个程序，或者告诉斗地主程序发生了异常，下一步请自行处理······

你可以分析这是个很严密的过程，保证了用户程序不会破坏其他用户程序和操作系统以及IO设备

#### [<死锁专题>]

![image-20200913175210064](X:\Users\xu\AppData\Roaming\Typora\typora-user-images\image-20200913175210064.png)

![image-20200913175232272](X:\Users\xu\AppData\Roaming\Typora\typora-user-images\image-20200913175232272.png)

![image-20200913175258831](X:\Users\xu\AppData\Roaming\Typora\typora-user-images\image-20200913175258831.png)

![image-20200913175327122](X:\Users\xu\AppData\Roaming\Typora\typora-user-images\image-20200913175327122.png)
...
1. **互斥条件**：进程A占用某一个资源时，另外的任何进程不可以访问。
2. **请求和保持条件**：当进程A去请求资源而阻塞时，对已获得的资源保持不放。
3. **不剥夺条件**：进程B在占用一个资源时，不可被线程A剥夺，必须在使用完时由自己释放。
...
1. ##### 预防死锁----- 设计一种系统，确保系统永远不会进入死锁状态
   ...
   **破坏请求条件**：一次性分配进程所有所需要的资源，这样就不会再有请求了；

   **破坏保持条件**：只要有一个资源得不到分配，也不给这个进程分配其他的资源：

   **破坏不可剥夺条件**：

   * 如果进程A在申请资源时被拒绝，那么必须释放；
   * 线程A请求线程B所占用的资源时，操作系统可以抢占另外一个进程，要求他释放资源。

   **破坏环路等待条件**：通过定义资源类型的线形顺序来预防。系统给每类资源赋予一个编号，每一个进程按编号递增的顺序请求资源，释放则相反。一个进程只有获得较小编号的资源才能申请较大编号的资源。
...
![image-20200913185057751](X:\Users\xu\AppData\Roaming\Typora\typora-user-images\image-20200913185057751.png)

![image-20200913185121368](X:\Users\xu\AppData\Roaming\Typora\typora-user-images\image-20200913185121368.png)
...
* **共享内存**：可以解决消息队列通信中用户态与内核态之间数据**拷贝**过程带来的开销，**它直接分配一个共享空间，每个进程都可以直接访问**，就像访问进程自己的空间一样快捷方便，不需要陷入内核态或者系统调用，大大提高了通信的速度，享有**最快**的进程间通信方式之名。但是便捷高效的共享内存通信，**带来新的问题，多进程竞争同个共享资源会造成数据的错乱。**

* **信号量**：那么，就需要**信号量**来保护共享资源，以确保任何时刻只能有一个进程访问共享资源，这种方式就是互斥访问。**信号量不仅可以实现访问的互斥性，还可以实现进程间的同步**，信号量其实是一个计数器，表示的是资源个数，其值可以通过两个原子操作来控制，分别是 **P 操作和 V 操作**。

* **信号**：与信号量名字很相似的叫**信号**，它俩名字虽然相似，但功能一点儿都不一样。信号是进程间通信机制中**唯一的异步通信机制**，信号可以在应用进程和内核之间直接交互，内核也可以利用信号来通知用户空间的进程发生了哪些系统事件，信号事件的来源主要有硬件来源（如键盘 Cltr+C ）和软件来源（如 kill 命令），一旦有信号发生，**进程有三种方式响应信号 1. 执行默认操作、2. 捕捉信号、3. 忽略信号**。有两个信号是应用进程无法捕捉和忽略的，即 `SIGKILL` 和 `SEGSTOP`，这是为了方便我们能在任何时候结束或停止某个进程。
...
* 平衡树要求**每个节点的左子树和右子树的高度差至多等于1**，这个**要求实在是太严了**，导致每次进行插入/删除节点的时候，几乎都会破坏平衡树的第二个规则，进而我们都需要通过**左旋**和**右旋**来进行调整，使之再次成为一颗符合要求的平衡树。
  ...
  ![img](https://img2018.cnblogs.com/blog/1757082/201908/1757082-20190829170706946-1725854455.png)
...
前缀树有着广泛的应用，例如**自动补全**，**拼写检查**等等。我们将在后面的章节中介绍实际应用场景。
...
![search](https://img2018.cnblogs.com/blog/1519578/201907/1519578-20190724132128364-709713908.gif)
...
![search2](https://img2018.cnblogs.com/blog/1519578/201907/1519578-20190724132122536-1472744566.gif)
